...
import { DeleteLJModalContext } from "../pages/LearningJourneySkills"
export default function CourseDescriptionModal() {
  function deleteJourney(journey) {
    const backend_url = "http://localhost:5000"
    fetch(backend_url + "/learningjounery/delete/"+journey,
      {
        headers: {
  ...
  const { modal, setModal } = useContext(DeleteLJModalContext)
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                ...
                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  <button
                    type="button"
                    className="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-red-500 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-600 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                    onClick={() => deleteJourney(modal.journeyid)}
                    ref={cancelButtonRef}
                  >
                    Delete
                  </button>
                  <button
                    ...
                    onClick={() => setModal({ opened: false, name: modal.name, desc: modal.desc, journeyid: modal.journeyid })}
                    ref={cancelButtonRef}
                  >
                    Close
                  </button>
