...
const AdminEditRole = () => {
    ...
    function processEditRole(roleid) {
        if (added[0].length == 0) {
            ...
            const backend_url = "http://localhost:5000"
            fetch(backend_url + "/jobs/changeName",
                {
                    headers: {
            ...
            const post_body = {
                "job_id": roleID,
                "new_job_skills": newJobSkills,
            }
            fetch(backend_url + "/jobskill/edit",
                {
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
    ...
    const [added, setAdded] = useState([[], []]) //Default is [Job's Added Skills, Job's UnAdded Skills]
    var newRoleName = ''
    var addedSkills = []
    var unaddedSkills = []
    added[0] = addedSkills
    added[1] = unaddedSkills
    if (loading == false && skillLoading == false) {
            ...
            <div className="container-full grid grid-cols-5 grid-rows-6 max-h-screen">
                ...
                <div className='col-start-2 col-end-4 my-auto flex'>
                    ...
                    <div className="ml-4 font-bold relative">
                        ...
                        <input  defaultValue={rolename} onChange={(e) => newRoleName = e.target.value} type="text" id="roleName" placeholder="Role Name" className="w-full rounded-md border border-gray-200 shadow-sm sm:text-sm p-3" />
                ...
                <AddedContext.Provider value={added}>
                    <TableGroup />
                    <div onClick={() => processEditRole(roleid)} className='col-start-4 text-end col-end-6 row-start-6 row-end-6 my-auto overflow-y-auto max-h-screen mx-12 cursor-pointer'>
                        <a
        ...
        return <div className='grid grid-cols-3 grid-rows-3 container-full h-screen'><p className='my-auto text-center col-start-2 row-start-2'>loading...</p></div>
    }
}
