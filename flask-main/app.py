...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "mysql+mysqlconnector://root:" + "@localhost:3306/ljps_db"
)
        ...
        for file in dirs:
            if "course" in file:
                ...
                for index, row in file_df.iterrows():
                    ...
                    Course_Desc = row["Course_Desc"].replace("\x92s", "'s")
                    ...
                    if course_OBJ:
                        ...
                        course_OBJ.Course_Desc = Course_Desc
                        course_OBJ.Course_Status = Course_Status
                ...
                for index, row in file_df.iterrows():
                    ...
                    staff_OBJ = Staff.query.filter_by(Staff_ID=Staff_ID).first()
                    if staff_OBJ:
                        staff_OBJ.Staff_FName = Staff_FName
                        staff_OBJ.Staff_LName = Staff_LName
                        staff_OBJ.Dept = Dept
                        staff_OBJ.Email = Email
                        staff_OBJ.role = role
                        updated_staffs.append(staff_OBJ)
                        db.session.commit()
                    else:
                        new_staff = Staff(
                ...
                for index, row in file_df.iterrows():
                    ...
                    if type(Completion_Status) == float:
                        Completion_Status = "Not Applicable"
                    registration_OBJ = Registration.query.filter_by(
                        Reg_ID=Reg_ID
                    ).first()

                    if registration_OBJ:
                        registration_OBJ.Reg_ID = Reg_ID
                        registration_OBJ.Course_ID = Course_ID
                        registration_OBJ.Staff_ID = Staff_ID
                        registration_OBJ.Reg_Status = Reg_Status
                        registration_OBJ.Completion_Status = Completion_Status
                        updated_registrations.append(registration_OBJ)
                        db.session.commit()
                    else:
            ...
            jsonify(
                {
                    "import_information": {
                        "course": {
                            "new": len(new_courses),
                            "update existing": len(updated_courses),
                        },
                        "role": {
                            "new": len(new_roles),
                            "update existing": len(updated_roles),
                        },
                        "staff": {
                            "new": len(new_staffs),
                            "update existing": len(updated_staffs),
                        },
                        "registration": {
                            "new": len(new_registrations),
                            "update existing": len(updated_registrations),
                        },
                    }
                }
            ),
...
def updateCourseSkill():
    try:
        ...
        return (
            jsonify({"message": "Editing course skills failed.", "error": str(e)}),
            500,
        )
...
def skillDelete(skill_id):
    skill_obj = Skill.query.filter_by(Skill_ID=skill_id).first()
    if skill_obj:
        course_skills_objs = CourseSkill.query.filter_by(Skill_ID=skill_id)
        job_skills_objs = JobSkill.query.filter_by(Skill_ID=skill_id)
        try:
            ...
            print(e)
            return jsonify({"message": "Deleting skill failed.", "error": str(e)}), 500
    else:
        return jsonify({"message": "Error, skill not found."}), 404
...
def jobDelete(job_id):
    job_obj = Job.query.filter_by(Job_ID=job_id).first()
    if job_obj:
        JobSkill_objs = JobSkill.query.filter_by(Job_ID=job_id)
        LearningJounery_objs = LearningJounery.query.filter_by(Job_ID=job_id)
        ...
        try:
            ...
            return (
                jsonify(
                    {"message": "Sucessfully deleted job.", "data": {"job_id": job_id}}
                ),
                200,
            )
            ...
            print(e)
            return jsonify({"message": "Deleting job failed.", "error": str(e)}), 500
    else:
        return jsonify({"message": "Error, job not found."}), 404


@app.route("/learningjounery/delete/<int:lj_id>", methods=["DELETE"])
def skillLearningJounery(lj_id):
    lj_obj = LearningJounery.query.filter_by(LearningJounery_ID=lj_id).first()
    if lj_obj:
        lrc_objs = LearningJouneryCourses.query.filter_by(LearningJounery_ID=lj_id)
        try:
            for obj in lrc_objs:
                db.session.delete(obj)
            ...
            return (
                jsonify(
                    {"message": "Deleting learningjounery failed.", "error": str(e)}
                ),
                500,
            )
    else:
        return jsonify({"message": "Error, learningjounery not found."}), 404
